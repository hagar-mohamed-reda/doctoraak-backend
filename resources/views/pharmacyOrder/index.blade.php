@extends("layout.app", ["title" => __('pharmacy order')])


@php
$builder = (new App\PharmacyDoctor())->getViewBuilder();
@endphp
...
<div class="w3-padding" style="padding-bottom: 0px" >
    <ol class="breadcrumb shadow w3-round w3-white">
        ...
        <li class="active">{{ __('pharmacy order') }}</li>
    </ol>
</div>
        ...
        <div class="box-body" >
            ...
            <div class="w3-row" >
                <div class="w3-col l3 m4 s6 w3-padding" > 
                    <label>{{ __('search with pharmacy order info') }}</label>
                ...
                <div class="w3-col l3 m4 s6 w3-padding" > 
                    <label>{{ __('pharmacy') }}</label>
                    <select class="form-control select2" onchange="filter.filter.pharmacy_id=this.value"  > 
...
<button class="btn btn-primary btn-flat modal-trigger"
        data-toggle="modal"
        data-target="#addModal"  >{{ __('add pharmacy order') }}</button> 
@endsection
    ...
    <div class="modal-dialog modal-" role="document" >
        <div class="modal-content w3-display-container">
            <div class="modal-header bg-primary">
                ...
                <center class="modal-title w3-xlarge">{{ __('pharmacy order') }}</center>
            </div>
...
<script>
    ...
    function search() {
        var url = "{{ url('/') }}/pharmacyorder/data?" + $.param(filter.filter);
