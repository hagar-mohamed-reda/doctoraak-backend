<form class="form" method="post" action="{{ url('/lab/working_hours/update/') }}/{{ $lab->id }}"  >
    @csrf
    <table class="w3-table table-bordered text-center" >
        ...
        <tr  class=""  >
            ...
            <td>  
                    <input type="hidden" name="active[]" id="editInputActive{{ $lab->id }}{{ $item->id }}" value="{{ $item->active }}">
                    
                    <input type="checkbox" 
                           class="shadow hidden" 
                           id="editActive{{ $lab->id }}{{ $item->id }}" 
                           placeholder="active"  
                           {{ $item->active==1? 'checked' : '' }}
                           onclick="this.checked ? $('#editInputActive{{ $lab->id }}{{ $item->id }}').val(1) : $('#editInputActive{{ $lab->id }}{{ $item->id }}').val(0)">
                    <label for="editActive{{ $lab->id }}{{ $item->id }}" class="switch "></label>
