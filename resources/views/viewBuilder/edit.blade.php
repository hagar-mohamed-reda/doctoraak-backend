    ...
    <div class="box-body row">
        ...
        <div class="form-group w3-padding {{ $col['col'] == null? 'col-lg-4 col-md-4 col-sm-6 col-xs-12' : $col['col'] }} ">
            ...
            <select
                ...
                onchange="$(this).parent().find('input[type=hidden]').val($('#edit{{ $col['id'] }}').val())"
                multiple="multiple"
                id="edit{{ $col['id'] }}"  >  
                @foreach($col["data"] as $data)
                <option 
        ...
        <div class="form-group w3-padding {{ $col['col'] == null? 'col-lg-4 col-md-4 col-sm-6 col-xs-12' : $col['col'] }} "   >
            ...
            <label for="{{ $col['id'] }}">{{ $col['label'] }}</label>
            <br>
            <input type="hidden" id="lat2" name="lat" value="{{ optional($builder['model'])->lat }}"  >
            <input type="hidden" id="lng2" name="lng" value="{{ optional($builder['model'])->lng }}"    >
            <div class="form-control cursor" onclick="$('.map-modal').show()" >
                <span class="fa fa-map-marker w3-large" ></span>
                <span> {{ $col['label'] }} </span> 
            </div>
            <p class="w3-tiny w3-text-gray" id="editLatLngPlace{{$col['label']}}" ></p>
            <br>
            ...
            <script>
                function initMap2() {
                    ...
                    placeMarker = function (position, map) {
                        ...
                        map.panTo(position);
                        //
                        $('#editLatLngPlace{{$col['label']}}').html(position.lat()+"-"+position.lat());
                    }
                    @if ($builder['model'])
                        @if (optional($builder['model'])->lat)
                        placeMarker(new google.maps.LatLng({{ optional($builder['model'])->lat }}, {{ optional($builder['model'])->lng }}), map);
                        @endif
                    @endif
                }
            </script>
...
async defer></script>
        ...
        <div class="form-group w3-padding {{ $col['col'] == null? 'col-lg-4 col-md-4 col-sm-6 col-xs-12' : $col['col'] }} ">
            <label for="edit-{{ $col['id'] }}">{{ $col['label'] }}</label>
            <input type="hidden" name="{{ $col['name'] }}" id='edit-{{ $col["id"] }}' >
            <div class="{{ $col['class'] }}" id='rate-edit-{{ $col["id"] }}'  >

            </div>
        </div>
        <script>
            $(document).ready(function(){
                var redit{{ $col["id"] }} = new Ratebar(document.getElementById('rate-edit-{{ $col["id"] }}'));
                
                redit{{ $col["id"] }}.setOnRate(function(){
                    document.getElementById('edit-{{ $col["id"] }}').value = redit{{ $col["id"] }}.value;
                });
                redit{{ $col["id"] }}.rate({{ $col['value'] }});
            });
        </script>
