<?php

namespace App\Http\Controllers\Api\Doctor;

use App\Doctor;
use App\DoctorInsurance;
use App\Message;
use App\helper\Helper;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Hash;
use DB;

use App\View;

class AuthController extends Controller
{
    /**
     * resgister doctor api
     *
     * @param Request $request
     * @return void
     */
    public function register(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'name' => 'required',
      //    'email' => 'required',
            'phone' => 'required|max:11',
            'password' => 'required|min:8',
       //   'birthdate' => 'required|date|before_or_equal:'.\Carbon\Carbon::now()->subYears(18)->format('Y-m-d'),
            'gender' => 'required',
            'specialization_id' => 'required|numeric',
            'degree_id' => 'required|numeric',
        //  'cv' => 'required',
        ]);
     
        //return $request->file("cv");

        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }
        
        $phone = $request->phone;
        $phone2 = $request->phone2;

      
        // check phone
        if (Doctor::where("phone", $request->phone)->count() > 0)
            return Message::error(Message::$PHONE_UNIQUE,  null ,Message::$PHONE_UNIQUE_EN);

        // check if it is medical center
        if ($request->has("is_medical_center") && $request->is_medical_center) {
            $phone = $phone . '-' . $request->specialization_id;
        }
        
        
        try {
            $user = Doctor::create([
                'name' => $request->name,
                'email' => $request->email,
                'phone' => $phone, 
                'phone2' => $phone2,
                'gender' => $request->gender,
                'title' => $request->title,
                'description' => $request->description,
                'specialization_id' => $request->specialization_id,
                'degree_id' => $request->degree_id,
                'password' => bcrypt($request->password),
                'sms_code' => rand(11111,99999),
                'active' => 0,
            ]);

            if ($request->hasFile('photo')) {
                $user->photo = Helper::uploadImg($request->file("photo"), "/doctor/");
            }
            if ($request->hasFile('cv')) {
                $user->cv = Helper::uploadFile($request->file("cv"), "/doctor/");
            }
            
            if ($request->has("is_medical_center") && $request->is_medical_center) {
                $user->is_medical_center = $request->is_medical_center;
                
                $phone = explode("-", $user->phone)[0]? explode("-", $user->phone)[0] : '';
            }

            // convert json array to array
            $insurances = json_decode($request->insurance);
            if ($insurances != null) {
                foreach ($insurances as $insurance) {
                    $d = new DoctorInsurance;
                    $d->doctor_id = $user->id;
                    $d->insurance_id = $insurance;

                    $d->save();
                }
            }

            $user->save();
    

            // send sms message
            Helper::sendSms("Your Code is ".$user->sms_code, $phone);
            return Message::success(Message::$SUCCESS_REGISTER,$user->getJson(),Message::$SUCCESS_REGISTER_EN);
        } catch (Exception $e) {
            return Message::error(Message::$ERROR . $e , null,Message::$ERROR_EN);
        }
    }

    /**
     * verfiy doctor account with sms code
     *
     * @param Request $request
     * @return void
     */
    public function verify_account(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'user_id' => 'required|numeric',
            'sms_code' => 'required',
        ]);
        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }
        try {
            $user = Doctor::find($request->user_id);

            if ($user->sms_code != $request->sms_code) {
                return Message::error(Message::$SMS_CODE_ERROR,  null,Message::$SMS_CODE_ERROR_EN);
            } else {
                $user->api_token = str_random(rand(20, 100));
                $user->active = 1;
                $user->update();
                return Message::success(Message::$VERIFIED, $user->getJson(),Message::$VERIFIED_EN);
            }
        } catch (\Exception $e) {
            return Message::error(Message::$ERROR,  null, Message::$ERROR_EN);
        }
    }
    
     public function resend(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'user_id' => 'required|numeric', 
        ]);
        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }

        try {
            $user = Doctor::find($request->user_id);
            Helper::sendSms("Your Code is ".$user->sms_code, $user->phone);
            return Message::success(Message::$VERIFIED, $user->getJson(),Message::$VERIFIED_EN);
            
           
        } catch (\Exception $e) {
            return Message::error(Message::$ERROR,  null,Message::$ERROR_EN);
        }
    }
      
      
      public function resend_two(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'phone' => 'required|numeric',
            
           
        ]);
        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }

        try {
            
            $user = Doctor::where('phone', $request->phone)->first();
             Helper::sendSms("Your Code is ".$user->sms_code, $user->phone);
              return Message::success(Message::$VERIFIED, $user->getJson(),Message::$VERIFIED_EN);
            
           
        } catch (\Exception $e) {
            return Message::error(Message::$ERROR,  null,Message::$ERROR_EN);
        }
    }

    /**
     * login Doctor api
     *
     * @param Request $request
     * @return void
     */
    public function login(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'phone' => 'required',
            'password' => 'required',
        ]);
        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }

        try {
            $user = Doctor::where('phone', $request->phone)->first();

            if (!$user)
                return Message::error(Message::$PHONE_ERROR_LOGIN,null,Message::$PHONE_ERROR_LOGIN_EN);
            
            if (Hash::check($request->password, $user->password)) {

                if ($user->active != 1) {
                    return Message::error(Message::$ACTIVE_NULL,null,Message::$ACTIVE_NULL_EN);
                }

                $user->api_token = str_random(rand(20, 100));
                $user->update();

                // add view for application
                View::create([ 
                    'date' => date("y-m-d"),
                    'ip' => $user->api_token 
                ]);
                return Message::success(Message::$SUCCESS_LOGIN, $user->getJson(),Message::$SUCCESS_LOGIN_EN);
            } else {
                return Message::error(Message::$PHONE_ERROR_LOGIN,null,Message::$PHONE_ERROR_LOGIN_EN);
            }
        } catch (\Exception $e) {
            return Message::error(Message::$ERROR,  null,Message::$ERROR_EN);
        }
    }
    /**
     *  send new password in sms to doctor phone
     *
     * @param Request $request
     * @return void
     */
    public function forget_password(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'phone' => 'required|max:11',
        ], []);
        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }

        try {
            $user = Doctor::where('phone', $request->phone)->first();
            if (!$user) {
                return Message::error(Message::$PHONE_ERROR,null,Message::$PHONE_ERROR_EN);
            }

           $newPassword = rand(11111, 99999);
            $user->update([
                //'password' => bcrypt($newPassword),
                'sms_code' => $newPassword
                ]);

            // send newPassword
            $smsMessage = str_replace("password", $newPassword, Message::$NEW_PASSWORD);
            Helper::sendSms($smsMessage, $user->phone);

            return Message::success(Message::$PASSWORD_SENT,null,Message::$PASSWORD_SENT_EN);
        } catch (\Exception $e) {
            return Message::error(Message::$ERROR,  null,Message::$ERROR_EN);
        }
    }


    /**
     * udpate patient password based on sms password
     *
     * @param Request $request
     * @return void
     */
    public function update_password(Request $request)
    {
        $validator = validator()->make($request->all(), [
            'phone' => 'required|max:11',
            'old_password' => 'required',
            'new_password' => 'required|min:8',
        ], []);
        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }

        try {
            $user = Doctor::where('phone', $request->phone)->first();
            if (!$user) {
                return Message::error(Message::$PHONE_ERROR,null,Message::$PHONE_ERROR_EN);
            }

            if ($request->old_password == $user->sms_code) {//(Hash::check($request->old_password, $user->password)) {
                // set new password
                $user->password = bcrypt($request->new_password);
                $user->update();
                return Message::success(Message::$EDIT,null,Message::$EDIT_EN);
            } else
                return Message::error(Message::$PASSWORD_NOT_CORRECT,null,Message::$PASSWORD_NOT_CORRECT_EN);
        } catch (\Exception $e) {
            return Message::error(Message::$ERROR,null,Message::$ERROR_EN);
        }
    }


    /**
     * update doctor profile info
     *
     * @param Request $request
     * @return void
     */
    public function update_profile(Request $request)
    {

        // $data = json_decode($request->getContent());

        $validator = validator()->make($request->all(), [
            'user_id' => 'required',
            'api_token' => 'required',
        ]);


        if ($validator->fails()) {
            return Message::error($validator->errors()->first());
        }

        if (Doctor::where("api_token", $request->api_token)->count() <= 0)
            return Message::error(Message::$API_LOGIN,null,Message::$API_LOGIN_EN);
        try {

            $user = Doctor::find($request->user_id);
            $user->name = $request->name;
            $user->degree_id = $request->degree_id;
            $user->phone2 = $request->phone2;
            $user->title = $request->title;
           
           
            
             


            ///edit image
            if ($request->hasFile('photo')) {
                // delete old image
                try{
                     unlink(public_path("image/doctor") . "/" .  $user->photo);
               
                } catch (\Exception $e) {
              }
                // upload new image
                $user->photo = Helper::uploadImg($request->file("photo"), "/doctor/");
            }
            ///edit cv
            if ($request->hasFile('cv')) {
                // delete old image
                try{
                    unlink(public_path("image/doctor") . "/" .  $user->cv);
               
                } 
                catch (\Exception $e) {
              }
                // upload new image
                $user->cv = Helper::uploadFile($request->file("cv"), "/doctor/");
            }
            // if we have new insurance  company delete the old and add new one
            DB::statement("delete from doctor_insurances where doctor_id='$user->id' ");
            $insurances = json_decode($request->insurance);
            if ($insurances != null) {
                foreach ($insurances as $insurance) {
                    $d = new DoctorInsurance;
                    $d->doctor_id = $user->id;
                    $d->insurance_id = $insurance;

                    $d->save();
                }
            }

            $user->update();
            return Message::success(Message::$EDIT,$user->getJson(),Message::$EDIT_EN);
        } catch (\Exception $exc) {
            return Message::error(Message::$ERROR . $exc->getMessage(), null);
        }
    }
}
